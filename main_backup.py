...
class NowPlayingSheet:
    ...
    def _ensure_initialized(self):
        ...
        show_feedback = getattr(self.app, "auto_dj_mode", False) or in_mood_partition
    ...
    def update_play_similar(self, enabled: bool):
        ...
        show_feedback = in_mood_partition or getattr(self.app, "auto_dj_mode", False)
...
class StreamripFletApp:
    ...
    async def _similar_auto_continue_queue(self):
        """Automatically extend the Play Similar queue with 5 acoustically
        similar tracks silently (without Jarvis voice speaks or UI bubbles)
        when playback reaches the end of the current list."""
        import asyncio
        import os
        from utils import track_graph as tg
        ...
        if self._session_bad_paths:
            try:
                blobs = await self.db_manager.get_embeddings_for_paths(list(self._session_bad_paths))
                for blob in blobs.values():
                    v = tg._unpack_embedding(blob)
                    if v is not None:
                        negative_embs.append(v)
            except Exception as exc:
                logger.debug("Play Similar continuation: negative emb load failed: %s", exc)
