...
class AssistantRunner:
    ...
    async def _resolve_query(self, query: str) -> Optional[dict]:
        """Find the best local-library match for a free-text query. Returns
        a track row dict (with title/artist/album/path) or None."""
        if not query:
            return None
    ...
    async def _handle_play_now(self, intent: ai.Intent) -> AssistantResponse:
        ...
        return AssistantResponse(
            spoken=f"{self._say('affirmative')} Playing {track.get('title')} by {track.get('artist')}.",
            displayed=f"Certainly. Now playing: **{track.get('title')}** — {track.get('artist')}",
            extras={"track": track},
        )
