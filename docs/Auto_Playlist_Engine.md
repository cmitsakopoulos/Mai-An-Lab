...
Vocal commands like *"play something chill"* or *"play happy tracks"* utilize a dynamic, library-relative scoring algorithm. The canonical vocabulary lives in a single `MOODS` table; `MOOD_PROFILES` and `MOOD_KEYWORDS` are derived views kept for backwards compatibility (do not edit them directly).
...
A custom mood is created by averaging the 52-D timbre vectors of a handful of exemplar tracks — that average is the **centroid**. For a track to match a custom mood, the scorer computes the cosine similarity between its timbre BLOB and the centroid:
